...
export function CanvasStage({
  ...
  return (
    ...
    >
          ...
          {motifs.map((m) => (
            <button
              ...
              className={`absolute flex -translate-x-1/2 -translate-y-1/2 cursor-grab touch-none select-none items-center justify-center rounded-md p-1 text-5xl leading-none transition-shadow active:cursor-grabbing ${
                selectedId === m.id
                  ? "ring-2 ring-[#C49A5C] ring-offset-2 ring-offset-[#F4E7D3]"
                  : ""
              }`}
              style={{
                left: `${m.x}%`,
      ...
      {dropActive && (
        <div className="pointer-events-none absolute inset-4 rounded-xl border-2 border-dashed border-[#C49A5C]" />
      )}
    </div>
  )
}
...
function Peg({ className }: { className: string }) {
  return (
    <div
      ...
      aria-hidden="true"
    >
      <span className="size-3 rounded-full border-2 border-[#C49A5C] bg-[#1E1C1A]" />
